...
const { getFileBuffer, colors, uploader } = require(`${process.cwd()}/exports.js`)
...
const paidhourspath = `./arquivos/commands/horarios/horarios.json`;
...
const paidgrouplinkpath = `./arquivos/commands/horarios/grouplink.json`
...
async function addGroupLinkInPaid(blackmd, from) {
    if(!isJsonIncludes(groupLinkPaid, from)) {
        ...
        groupLinkPaid.push({id: from, foto: `https://files.catbox.moe/ydc580.jpg` })
        paidSGL()
        }
    }
}
...
async function paidFunc(sabrina) {
    if(paidHours.length > 0) {
        ...
        if(totalpaid > 0) {
            ABC = {dica: `*Dica:* Alterne entre rodadas tradicionais e rápidas. Ao atingir um \"Grande Prêmio\", PARE imediatamente e espere o momento certo para retornar!\n• _Os horários de maior probabilidade podem aumentar suas chances de sucesso, mas não garantem vitórias. Mesmo com chances favoráveis, é fundamental jogar de forma consciente e dentro dos seus limites. Boa sorte!_`, resultado: gerarHorariosAleatorios(obterHorarioAtual().split(':')[0], 0, 59)}
            for(a of paidHours) {
                from = a.groupId
                    ...
                    for(b of a.horarios) {
                        if(b.dias <= 0) {
                            if(contarMin(sendHours(`HH:mm`)) >= contarMin(b.tempo)) {
                                ...
                                sabrina.sendMessage(from, {image: {url: getGroupLinkFromPaidID(from)}, caption: `🍀Horários pagantes das ${sendHours("HH")} horas:\n• O uso das BETs *(plataformas de apostas)* é proibido para pessoas menores de 18 anos, segundo a lei 14.790/2023 _(29 de Dezembro de 2023)_\n—\n${ABC.resultado.map(h => `*${h.name}*\n${h.times.map(p => `⥲ ${p}`).join(`\n`)}`).join(`\n\n`)}\n—\n${ABC.dica}`, contextInfo: {forwardingScore: 999, isForwarded: true}})
                            }
                        } else {
